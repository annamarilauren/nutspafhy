...
from spafhy_io import preprocess_soildata, read_AsciiGrid
...
class SpaFHy():
    ...
    def run_timestep(self, forc, current_datetime, ncf=False, flx=False):
        """ 
        ...
        # save infi_ex (surface runoff from each pixel -> save to netcdf!, unit m dt-1)
        #infi, infi_ex, drain, tr, eva, drainretflow, mbes = self.bu.watbal(dt=self.dt, rr=1e-3*potinf, tr=1e-3*transpi,
        #                                               evap=1e-3*efloor, retflow=qr)


        # run BucketGrid water balance
        #corrected 20 05 2020
        ...
        if ncf:
            ...
            k = self.step_nr
            time_unit_out = "days since 0001-01-01 00:00:00.0"
            
            ncf['time'][k] = date2num(current_datetime,time_unit_out, calendar='standard')
            #print (current_datetime, ncf['time'][k], k)
            #print (num2date(ncf['time'][k], time_unit_out, calendar='standard'))
            #import sys; sys.exit()
            # canopygrid
            ...
            ncf['bu']['Drainretflow'][k,:,:] = None #self._to_grid(drainretflow)
            ...
            ncf['bu']['Porosity'][:,:] = self._to_grid(self.bu.poros)
            ncf['bu']['Infi_ex'][k,:,:] = self._to_grid(roff)
    ...
    def set_clear_cut(self, cut):
        """ input raster: clear cut as 1"""
        try:
            ixc = np.where(cut>0)
            self.bu.PondSto[ixc] += 1e-3*self.cpy.W[ixc]                        #canopy storage to soil pond storage
            self.cpy.W[ixc] = 0.0
            self.cpy.hc[ixc] = 0.1
            self.cpy.cf[ixc] = 0.01
            self.cpy._LAIconif[ixc]=0.1
            self.cpy._LAIdecid_max[ixc]=1.0            
        except:
            'Failure in setting thee clear-cut'
...
def initialize_netCDF(ID, fname, lat0, lon0, dlat, dlon, dtime=None):
    ...
    Drainretflow = ncf.createVariable('/bu/Drainretflow', 'f4', ('dtime', 'dlat', 'dlon',))
    Drainretflow.units = 'drainage and returnflow [mm]'   
    Porosity = ncf.createVariable('/bu/Porosity', 'f4', ('dlat', 'dlon',))
    Porosity.units = 'soil porosity [m3 m-3]'       
    Infi_ex = ncf.createVariable('/bu/Infi_ex', 'f4', ('dtime', 'dlat', 'dlon',))
    Infi_ex.units = 'surface runoff [mm]'
